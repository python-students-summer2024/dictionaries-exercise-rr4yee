...
def bake_cookies(filepath):
    ...
    for line in lines[1:]:
        ...
        dict = {"id": int(data[0]), "title": data[1], "description": data[2], "price": float(data[3].replace("$", "")), "contains_nuts": data[4].lower() == 'true', "gluten_free": data[5].lower =='true', "sugar_free": data[6].lower() == 'true'}
...
def display_cookies(cookies):
    ...
    nuts = input("Are you allergic to nuts? (please enter yes or no) ").lower() in ['yes', 'y']
    gluten = input("Are you allergic to gluten? (please enter yes or no) ").lower() in ['yes', 'y']
    sugar = input("Do you suffer from diabetes? (please enter yes or no) ").lower() in ['yes', 'y']
    for cookie in cookies:
        if (not nuts or not cookie["contains_nuts"]) and (not gluten or not cookie["gluten_free"]) and (not sugar or cookie["sugar_free"]):
            restrictions.append(cookie)
    print("Great! Here are the cookies without nuts or sugar that we think you might like:")
    for cookie in restrictions:
